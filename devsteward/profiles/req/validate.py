...
@dataclass
class Signoff:
    """A human verdict on one ``manual`` AC (Decision 4): the checkbox is the human's —
    ``approved`` plus an optional one-line ``scope`` — and the engine composes the rest
    of the provenance (date, reviewer line, event detail) mechanically."""
...
class ReqValidateRoutine:
    ...
    def _park_red(
        self, ex, step: Step, req: ReqFile, results: list[dict], *, in_flight: bool
    ) -> StepResult:
        """Red validation parks immediately with the failure brief — no repair loop
        (Decision 8): a red here is a human question (hollow develop or broken lab)."""
        led = ex.ledger
