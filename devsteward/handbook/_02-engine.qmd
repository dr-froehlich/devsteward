...
repair loop. A `manual` criterion is a **guided** human validation (REQ-034): `steward
validate` runs a two-phase bookkeeping around a foreground *interactive* session attached
to the terminal (the editor pattern — never `claude -p`, never spawned from within Claude),
and a pending verdict parks as **async QA** — clean tree, HEAD back on the integration
branch, the feature branch intact and unmerged — so the project keeps moving and the
validation resumes (reconciling the branch) later. See @sec-workflow for the full phase.
