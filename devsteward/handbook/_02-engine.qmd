...
*additionally* refuses unless a **concept doc** at `docs/concepts/REQ-NNN.md` exists and the
REQ's `concept_refs:` names it (REQ-039 — concept-first, the same existence check one altitude
up; the attended develop session *is* the concept phase, so there is no separate concept step).
