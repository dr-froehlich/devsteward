...
- **A pending validation is async QA, not a project freeze (REQ-034).** When the human
  defers, the validation parks cleanly — clean working tree, HEAD back on the integration
  branch, the feature branch intact and **unmerged** (no merge gates the unvalidated work
  onto `dev`) — and `steward run` advances other eligible REQs; the waiting human is a
  standing work-item, not a blocked pipeline. `steward validate REQ-NNN` resumes it at any
  later time, **reconciling** the feature branch from the (now-advanced) integration branch
  rather than refusing it as diverged, and a green verdict fires the deferred land. A
  declined verdict is a red validation that routes to `steward rework` (below).
