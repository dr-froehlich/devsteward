...
REQ-001
 ...
 REQ-030 + REQ-033 + REQ-034 + REQ-055 + REQ-075 ── REQ-081 (feature: warm validation cycle — the REQ-034 start/record halves become real CLI verbs [`steward validate start/record`]; start runs warm in the System-Tester session [no CLAUDECODE refusal — it spawns nothing], the verdict is recorded from a second plain shell via the engine's own interactive prompt [`!` channel has no stdin, tested 2026-07-17; flags rejected — firewall], rework/revalidate from that same shell, then the still-warm session re-runs start scoped per REQ-075; shape A unchanged; no new guards or locks [REQ-079 doctrine holds by discipline])
 REQ-016 + REQ-025 + REQ-058 + REQ-059 ── REQ-080 (fix: steward run rides through a budget limit — an ambiguous ERROR death is corroborated via clauder gate [oracle, not a bigger marker list], LIMIT no longer terminates the run loop [the existing precheck wait/re-gate/switch owns the waiting], the interrupted step requeues RECOVER so the relaunch carries --repeat, 3-strike consecutive-limit guard, clauder-absent degrades to today's stop; from the FlowSteward REQ-116 quit, 2026-07-16)
 REQ-082 (feature: steward cache — standalone read-only CLI verb; newest session transcript mtime vs a 60m/--ttl threshold answers "is my Claude session still warm" from a second shell [asking inside the session would itself warm/cold-pay the cache]; slug = abs project root '/'→'-', CLAUDE_PROJECTS_DIR env seam for self-contained tests; mtime-only — prefix invalidation [model switch/compaction] stays a printed caveat, not a parsed signal)
